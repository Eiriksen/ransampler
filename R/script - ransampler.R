...
#' @export
ransampler = function(dataframe, ofEach,nOfEach,noShareWithin,priBy,useDuplis=F){

  dataframe$ID_num       <- 1:nrow(dataframe)
  table_combinations     <- combinationTable(dataframe, ofEach)
  table_foundIndividuals <- data.frame()
    ...
    for (row in 1:nrow(table_combinations)){
    {
      ...
      while (found==F)
      {
        ...
        if (nrow(table_thisCombinationsIndiv_narrow) == 0)
        {
          ...
          table_foundIndividuals = rbind(table_foundIndividuals,NA)
          next()
          }
  ...
  print(notFoundList)
  print(count_internalConflicts)
  print(count_externalConflicts)
  print("Done")
